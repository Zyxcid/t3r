...
# ════════════════════════════════════════════════════════
#  ICON GENERATOR (Pillow)
# ════════════════════════════════════════════════════════
def create_icon_image():
    img = Image.new('RGBA', (64, 64), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
    draw.ellipse([8, 8, 56, 56], fill=(61, 214, 140, 255)) # ACCENT_GREEN
    return img
...
class BreakOverlay:
    ...
    def _build(self):
        ...
        size = 280
        self.canvas = tk.Canvas(center, width=size, height=size,
                                bg=self.OVERLAY_BG, highlightthickness=0)
        self.canvas.pack()

        cx = cy = size // 2
        r  = 110
        self.canvas.create_oval(cx-r, cy-r, cx+r, cy+r,
                                outline=self.RING_EMPTY, width=10)
        self.arc = self.canvas.create_arc(
            cx-r+5, cy-r+5, cx+r-5, cy+r-5,
            start=90, extent=359.9,
            outline=self.RING_FG, width=10, style="arc")
        self.num_text = self.canvas.create_text(
            cx, cy - 14, text=str(self.remaining),
            fill=TEXT_PRIMARY, font=("Courier New", 72, "bold"))
        self.canvas.create_text(cx, cy + 54, text="sec",
                                fill="#2A7A4A", font=("Segoe UI", 14))

        skip = tk.Label(w, text="Skip  ×",
                        bg=self.OVERLAY_BG, fg=self.SKIP_FG,
                        font=("Segoe UI", 10), cursor="hand2")
        skip.place(relx=1.0, rely=1.0, anchor="se", x=-28, y=-24)
        skip.bind("<Button-1>", lambda e: self._skip())
        tk.Label(w, text="Press Esc to skip",
                 bg=self.OVERLAY_BG, fg=self.SKIP_FG,
                 font=("Segoe UI", 9)).place(relx=0.5, rely=0.96, anchor="center")
...
class Timer2020App:
    ...
    def _setup_icon_and_tray(self):
        ...
        menu = pystray.Menu(
            ...
            pystray.MenuItem("Start on Login", toggle_autostart, 
    ...
    def _update_tray_tip(self):
        ...
        if self.state == "WORK":
            ...
            self._tray.title = f"t3r | Focus {mins:02d}:{secs:02d}" # <-- Diubah
        elif self.state == "BREAK_OVERLAY":
            self._tray.title = "t3r | Eye break" # <-- Diubah
        else:
            self._tray.title = "t3r | Paused" # <-- Diubah

    # ── UI ──────────────────────────────────────────────
    def _build_ui(self):
        r = self.root

        card = tk.Frame(r, bg=BG_CARD,
                        highlightthickness=1, highlightbackground=BORDER)
        card.pack(fill="both", expand=True, padx=20, pady=20)

        self.state_label = tk.Label(card, text="FOCUSING",
                                    bg=BG_CARD, fg=TEXT_MUTED,
                                    font=("Segoe UI", 10, "bold"))
        self.state_label.pack(pady=(24, 0))

        self.time_label = tk.Label(card, text="20:00",
    ...
    def _btn(self, parent, text, cmd, color):
        return tk.Button(parent, text=text, command=cmd,
                         bg=BG_CARD, fg=color, relief="flat",
                         font=("Segoe UI", 10, "bold"),
    ...
    def _stat(self, parent, var, label):
        ...
        tk.Label(f, textvariable=var, bg=BG_CARD, fg=TEXT_PRIMARY,
    ...
    def _next_phase(self):
        if self.state == "WORK":
            ...
            threading.Thread(target=send_notification, daemon=True,
                args=("🔔 Eye Break!",
                      f"Look 20 feet (6 meters) away for {BREAK_SECONDS} seconds."),
            ).start()
    ...
    def _on_break_done(self):
        ...
        self.remaining     = WORK_SECS
        threading.Thread(target=send_notification, daemon=True,
            args=("✅ Back to Work!", "Break is over. Keep it up!"),
        ).start()
